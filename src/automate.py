...
def get_opgg_rune_indexs(champion, lane):
    ...
    elems = driver.find_elements_by_xpath("//img[contains(@src, 'opgg-static.akamaized.net/images/lol/perk') and not (contains(@src, 'grayscale')) and not(contains(@src, 'image=q_auto,w_')) and contains(@class, 'tip')]")
...
def get_league_window():
    try:
        return autoit.win_get_client_size('League of Legends')
    except Exception as e:
        messagebox.showerror('Error', str(e))
        return None
...
def apply_rune_set(name, rune_set, offset=(0, 0)):
    ...
    game_window_dim = get_league_window()
    ...
    # add offset position coords
    for i in range(len(coords)):
        for j in range(len(coords[i])):
            for k in range(len(coords[i][j])):
                coords[i][j][k][0] += offset[0]
                coords[i][j][k][1] += offset[1]

    # Change coords if other client resolution
    if game_window_dim != (1280, 720):
        print('modifying using ratio')
        game_width, game_height = game_window_dim
        ratio_x = game_width/1280
        ratio_y = game_height/720
        for i in range(len(coords)):
            for j in range(len(coords[i])):
                for k in range(len(coords[i][j])):
                    coords[i][j][k][0] = int(coords[i][j][k][0] * ratio_x)
                    coords[i][j][k][1] = int(coords[i][j][k][1] * ratio_y)

    # add game position coords
    for i in range(len(coords)):
        for j in range(len(coords[i])):
            for k in range(len(coords[i][j])):
                coords[i][j][k][0] += game_x
                coords[i][j][k][1] += game_y
    ...
    # rename rune page
    clipboard_backup = pyperclip.paste() # get current clipboard
    ...
    sleep(.5)
    keyboard_type('{ENTER}')
    pyperclip.copy(clipboard_backup) # reset clipboard
    sleep(1) # df: 2, sleep bc else the name won't be edited every where
    ...
    #win32api.SetCursorPos(initial_mouse_position) # this sucks


def test(name,offset=(0,0)):#-112, 5)):
    initial_mouse_position = win32api.GetCursorPos()
    current_game_window_position = get_league_pos()
    if current_game_window_position is None:
        return
    ...
    name_button_pos = [name_button_pos[0] + game_x + offset[0], name_button_pos[1] + game_y + offset[1]]
    name_entry_pos = [name_entry_pos[0] + game_x + offset[0] + 10, name_entry_pos[1] + game_y + offset[1]]

    mouse_click(name_button_pos)
    print('name entry button')
    sleep(3)

    #mouse_click(name_entry_pos)
    #print('name entry field')
    #sleep(1)
